...
export default function ResultsPage() {
  ...
  useEffect(() => {
    const raw = sessionStorage.getItem("vic-portfolio");
  ...
  if (!data) return (
    <div className="shell">
      <Sidebar />
      <main className="main">
        <div className="topbar"><div className="topbar-title">My portfolio</div></div>
        <div className="content"><div className="loading-wrap"><div className="spinner"></div><div className="loading-text">Loading…</div></div></div>
      </main>
    </div>
  );
          ...
          <div className="portfolio-card">
            ...
            {data.lines.map((l) => (
              <div key={l.isin} className="line-row">
                ...
                <div>
                  <div className="line-pct">{l.allocation_pct}%</div>
                </div>
                <div>
                  <div className="line-amount">{fmtChf(l.amount_chf)}</div>
                </div>
                <div>
                  <div className="line-ter">{l.ter_pct.toFixed(2)}%</div>
                </div>
              </div>
            ))}
          </div>
